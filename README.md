...
در این بخش نیز، مشابه قسمت قبلی، با توجه به اینکه کمی نحوه ایمپورت کردن فایل برای من، گنگ بود، از هوش مصنوعی کمک گرفتم تا فایل را بتوانم وارد برنامه بکنم، اما ارور های مورد نظر(مانند اینکه فایل فرمت درستی نداشته باشد، فایل Json باشد، اما فرمت درست برای برنامه ما نداشته باشد، اعمال نام فایل انتخابی در برنامه و ...) را کامل خودم پیاده کردم. مشابه Export، قابلیت Import را نیز در والد اصلی قرار می‌دهیم و به Header پاس می‌دهیم. با این کار، تغییرات مورد نظر روی نام فایل و متغیر shapes، راحت‌تر خواهد بود. در کد نوشته شده برای این بخش، با کلیک بر روی دکمه Import، یک سلکتور باز خواهد شد تا کاربر، بتواند فایل را انتخاب بکند، البته باید در نظر داشت که فرمت پیش‌فرض، Json است، اما در صورتی که کاربر، خودش تنظیمات را تغییر داده و فایلی با فرمت دیگری انتخاب بکند، با ارور روبه‌رو خواهد شد. در صورتی که فایل انتخابی، Json بوده، اما فرمت آرایه ای از اشکال را نداشته باشد نیز، کاربر با ارور روبه‌رو خواهد شد.

## بخش پایانی

در نهایت، تمامی موارد خواسته شده در صورت تمرین، در این پروژه پیاده سازی شده است، قابلیت Drage and drop نیز در برنامه موجود است و برای اشکال نیز، دو شکل اضافه تر در برنامه قرار داده شد تا ظاهر و جلوه بهتری داشته باشد. مواردی که در آن‌ها از هوش مصنوعی، کمک گرفته شده است، بدین شرح هستند:

- ترسیم اشکال هندسی(علت استفاده: فرمول های مورد نظر برای پیاده‌سازی آن‌ها را به صورت دقیق نیاز داشتم)
- برای قسمت Drop کردن اشکال بر روی بوم، به مشکل خورده بودم(اشکال را می‌توانستم انتخاب کنم و بر روی بوم ببرم، اما به هنگام Drop کردن، بر روی بوم، ذخیره نمی‌شد).
- قسمت های ابتدایی Export و Import
